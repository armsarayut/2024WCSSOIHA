...
{
    public class PaM67ARptPdf : PdfPageEvents
    {
        //-----------------------------
        ...
        /// <returns></returns>
        public byte[] ExportPDF(List<Class6_7_A> ListRpt)
        {
            PaM67ARptPdf pdfReport = new PaM67ARptPdf();
            Document document;
            if (bPageLanscape)
            {
        ...
        /// <returns></returns>
        private void ReportBody(Document document, List<Class6_7_A> ListRpts)
        {
            BaseFont baseFont = BaseFontForHeaderFooter;
            ...
            cell = new PdfPCell(new Phrase("TASKCOUNT", _fontstyeheader))
            {
                HorizontalAlignment = Element.ALIGN_LEFT,
                VerticalAlignment = Element.ALIGN_MIDDLE,
                BackgroundColor = headerBackcolor,
            ...
            foreach (var listRpt in ListRpts)
            {
                ...
                cell = new PdfPCell(new Phrase(Convert.ToDateTime(listRpt.W_Hour).ToString(VarGlobals.FormatDTHM), _fontstyebody))
                {
                    HorizontalAlignment = Element.ALIGN_LEFT,
                    VerticalAlignment = Element.ALIGN_MIDDLE,
                ...
                cell = new PdfPCell(new Phrase(string.Format(VarGlobals.FormatN0, listRpt.W_Count), _fontstyebody))
                {
                    HorizontalAlignment = Element.ALIGN_LEFT,
                    VerticalAlignment = Element.ALIGN_MIDDLE,
                    BackgroundColor = bodyBackcolor,
