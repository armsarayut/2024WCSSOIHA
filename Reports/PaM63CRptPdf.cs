...
{
    public class PaM63CRptPdf : PdfPageEvents
    {
        //-----------------------------

        #region Attributes
        readonly string reportCaption = "5.3.3.Inventory aging - Report";
        ...
        /// <returns></returns>
        public byte[] ExportPDF(List<Class6_3_C> ListRpt)
        {
            PaM63CRptPdf pdfReport = new PaM63CRptPdf();
            Document document;
            if (bPageLanscape)
            {
        ...
        /// <returns></returns>
        private void ReportBody(Document document, List<Class6_3_C> ListRpts)
        {
            BaseFont baseFont = BaseFontForHeaderFooter;
            ...
            cell = new PdfPCell(new Phrase("AGING(Day)", _fontstyeheader))
            {
                HorizontalAlignment = Element.ALIGN_LEFT,
                VerticalAlignment = Element.ALIGN_MIDDLE,
                BackgroundColor = headerBackcolor,
            ...
            foreach (var listRpt in ListRpts)
            {
                ...
                cell = new PdfPCell(new Phrase(string.Format(VarGlobals.FormatN2, listRpt.DisQty), _fontstyebody))
                {
                    HorizontalAlignment = Element.ALIGN_LEFT,
                    VerticalAlignment = Element.ALIGN_MIDDLE,
                    BackgroundColor = bodyBackcolor,
                ...
                cell = new PdfPCell(new Phrase(string.Format(VarGlobals.FormatN0, listRpt.Aging), _fontstyebody))
                {
                    HorizontalAlignment = Element.ALIGN_LEFT,
                    VerticalAlignment = Element.ALIGN_MIDDLE,
                    BackgroundColor = bodyBackcolor,
