...
{
    public class WhStockListRptPdf : PdfPageEvents
    {
        #region Attributes
        readonly string reportCaption = "2.1.Stocklist - Report";
        ...
        /// <returns></returns>
        public byte[] ExportPDF(List<Inv_Stock_GoInfo> ListRpt)
        {
        ...
        /// <returns></returns>
        private void ReportBody(Document document, List<Inv_Stock_GoInfo> ListRpts)
        {
            BaseFont baseFont = BaseFontForHeaderFooter;
            ...
            if (ListRpts.Count<=0)
            {
                cell = new PdfPCell(new Phrase("", _fontstyebody))
                {
                    HorizontalAlignment = Element.ALIGN_LEFT,
                    VerticalAlignment = Element.ALIGN_MIDDLE,
                    BackgroundColor = bodyBackcolor,
