...
{
    public class MasterUsersGroupRptPdf : PdfPageEvents
    {
        //-----------------------------

        #region Attributes
        readonly string reportCaption = "7.3.Group - Report";
        ...
        /// <returns></returns>
        public byte[] ExportPDF(List<Usergroups> ListRpt)
        {
            MasterUsersGroupRptPdf pdfReport = new MasterUsersGroupRptPdf();
            Document document;
            if (bPageLanscape)
            {
        ...
        /// <returns></returns>
        private void ReportBody(Document document, List<Usergroups> ListRpts)
        {
            BaseFont baseFont = BaseFontForHeaderFooter;
