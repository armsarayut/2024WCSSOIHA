...
{
    public class MasStatusPageRptPdf : PdfPageEvents
    {
        //-----------------------------

        #region Attributes
        readonly string reportCaption = "6.3.Status - Report";
        ...
        /// <returns></returns>
        public byte[] ExportPDF(List<Mas_Status_Go> ListRpt)
        {
            MasStatusPageRptPdf pdfReport = new MasStatusPageRptPdf();
            Document document;
            if (bPageLanscape)
            {
        ...
        /// <returns></returns>
        private void ReportBody(Document document, List<Mas_Status_Go> ListRpts)
        {
            BaseFont baseFont = BaseFontForHeaderFooter;
            ...
            foreach (var listRpt in ListRpts)
            {
                ...
                cell = new PdfPCell(new Phrase(listRpt.Statcode.ToString(), _fontstyebody))
                {
                    HorizontalAlignment = Element.ALIGN_LEFT,
                    VerticalAlignment = Element.ALIGN_MIDDLE,
