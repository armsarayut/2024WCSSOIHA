...
{
    public class PaM63ARptPdf : PdfPageEvents
    {
        //-----------------------------

        #region Attributes
        readonly string reportCaption = "5.3.1.Inventory location - Report";
        ...
        /// <returns></returns>
        public byte[] ExportPDF(List<Class6_3_A> ListRpt)
        {
            PaM63ARptPdf pdfReport = new PaM63ARptPdf();
            Document document;
            if (bPageLanscape)
            {
        ...
        /// <returns></returns>
        private void ReportBody(Document document, List<Class6_3_A> ListRpts)
        {
            BaseFont baseFont = BaseFontForHeaderFooter;
            ...
            foreach (var listRpt in ListRpts)
            {
                ...
                cell = new PdfPCell(new Phrase(listRpt.Shelfname.ToString(), _fontstyebody))
                {
                    HorizontalAlignment = Element.ALIGN_LEFT,
                    VerticalAlignment = Element.ALIGN_MIDDLE,
                    BackgroundColor = bodyBackcolor,
