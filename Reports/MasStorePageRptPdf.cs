...
{
    public class MasStorePageRptPdf : PdfPageEvents
    {
        //-----------------------------

        #region Attributes
        readonly string reportCaption = "6.5.Store - Report";
        ...
        /// <returns></returns>
        public byte[] ExportPDF(List<Mas_Storage_Go> ListRpt)
        {
            MasStorePageRptPdf pdfReport = new MasStorePageRptPdf();
            Document document;
            if (bPageLanscape)
            {
        ...
        /// <returns></returns>
        private void ReportBody(Document document, List<Mas_Storage_Go> ListRpts)
        {
            BaseFont baseFont = BaseFontForHeaderFooter;
            ...
            cell = new PdfPCell(new Phrase("STORECODE", _fontstyeheader))
            {
                HorizontalAlignment = Element.ALIGN_LEFT,
            ...
            cell = new PdfPCell(new Phrase("ADDRESS", _fontstyeheader))
            {
                HorizontalAlignment = Element.ALIGN_LEFT,
                VerticalAlignment = Element.ALIGN_MIDDLE,
                BackgroundColor = headerBackcolor,
            ...
            foreach (var listRpt in ListRpts)
            {
                ...
                cell = new PdfPCell(new Phrase(listRpt.Stocode.ToString(), _fontstyebody))
                {
                    HorizontalAlignment = Element.ALIGN_LEFT,
                    VerticalAlignment = Element.ALIGN_MIDDLE,
                ...
                cell = new PdfPCell(new Phrase(listRpt.Stoname.ToString(), _fontstyebody))
                {
                    HorizontalAlignment = Element.ALIGN_LEFT,
                    VerticalAlignment = Element.ALIGN_MIDDLE,
                    BackgroundColor = bodyBackcolor,
                ...
                cell = new PdfPCell(new Phrase(listRpt.Stoaddress.ToString(), _fontstyebody))
                {
                    HorizontalAlignment = Element.ALIGN_LEFT,
                    VerticalAlignment = Element.ALIGN_MIDDLE,
                    BackgroundColor = bodyBackcolor,
