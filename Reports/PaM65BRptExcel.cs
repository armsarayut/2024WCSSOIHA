...
{
    public class PaM65BRptExcel
    {
        MemoryStream _memoryStream = new MemoryStream();
        //List<Inb_Goodreceipt_Go> _Inb_Goodreceive_Go_s = new List<Inb_Goodreceipt_Go>();
        public byte[] Report(List<Class6_5_B> rptElements)
        {
            using (var workbook = new XLWorkbook())
            {
                var worksheet = workbook.AddWorksheet("5.5.2");
                ...
                worksheet.Cell("B1").Value = "5.5.2.ASRS-Error summary" + " - Report";
                ...
                worksheet.Cell(rptRows, 3).Value = "COUNT";
