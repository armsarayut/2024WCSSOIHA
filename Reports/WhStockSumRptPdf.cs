...
{
    public class WhStockSumRptPdf : PdfPageEvents
    {
        #region Attributes
        readonly string reportCaption = "2.1.Stock Summary - Report";
        ...
        private void ReportBody(Document document, List<InvStockSum> ListRpts)
        {
            ...
            foreach (var listRpt in ListRpts)
            {
                ...
                cell = new PdfPCell(new Phrase(string.Format(VarGlobals.FormatN2, listRpt.DisTotalstock).ToString(), _fontstyebody))
                {
                    HorizontalAlignment = Element.ALIGN_LEFT,
                    VerticalAlignment = Element.ALIGN_MIDDLE,
                    BackgroundColor = bodyBackcolor,
